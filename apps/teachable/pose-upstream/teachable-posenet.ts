...
export class TeachablePoseNet extends CustomPoseNet {
     ...
     */
    public prepare() {
        for (const classes of this.examples){
            if (classes.length === 0) {
    ...
    public async train(params: TrainingParameters, callbacks: CustomCallbackArgs = {}) {
        ...
        // Inputs for posenet
        const inputSize = this.examples[0][0].length;
        ...
        if (this.model.layers.length) this.model.dispose();
        ...
        */
        try { await this.model.fitDataset(trainData, {
